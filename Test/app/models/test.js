...
function updateParticipants(participants) {
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      var info_ele = '<div class="row user-row search_item" id="' + name + '">' + photo_ele + name_ele + dropdown_ele + '</div>';
      var detail_ele = '<div class="row user-info ' + name + '">' +
          '<button type="button"' +
          'name="'+name+'"' +
          'class="submitButton btn btn-info col-xs-6 col-sm-6 ' +
          'col-md-6 col-lg-6 col-xs-offset-3 col-sm-offset-3 col-md-offset-3' +
          ' col-lg-offset-3">chat</button><br><br>' +
          '<a href="#"' +
          'class="callPeer btn btn-info col-xs-6 col-sm-6 ' +
          'col-md-6 col-lg-6 col-xs-offset-3 col-sm-offset-3 col-md-offset-3' +
          ' col-lg-offset-3">Call</button>' +
          '<hr/></div></div>';
    ...
    $('#participants_online').off('click', '.callPeer');

    $('#participants_online').on('click', '.callPeer', function(e) {
      e.preventDefault();
      var callee = $(this).parents('div.row').siblings('div.search_item').attr('id');
      console.log('Initiate call to: ', callee);
      startOutgoingCall(callee);
    });
    ...
    participants.all.forEach(function(userObj) {
      for (var i=0; i<keys2.length; i++){
        ...
        if (userObj.userName == name){
          ...
          var detail_ele = '<div class="row user-info ' + userObj.userName + '"><button type="button" name="'+name+'" class="btn btn-info col-xs-6 col-sm-6 col-md-6 col-lg-6 col-xs-offset-3 col-sm-offset-3 col-md-offset-3 col-lg-offset-3 submitButton">chat</button><hr/></div></div>';
