...
$('body').on("click", ".edit_profile", function(){
	...
	$.ajax({
		...
		if(data.selected_user) {
			...
			selected_account_status = data.selected_user.account_status;
		}
	}).fail(function(){
	...
	submitHandler: function(form) {
		...
		$.ajax({
			...
			if(data.err_message) {
				...
				getAllUsers();
				if(new_username != selected_username || data.updated_user.account_status != selected_account_status) {
					client_socket.emit('update online users',{old_username : selected_username, new_username : new_username, updated_user : data.updated_user});
				}
			}
		}).fail(function(){
